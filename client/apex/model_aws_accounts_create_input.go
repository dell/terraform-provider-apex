/*
APEX Navigator for Multicloud Storage REST APIs

The public API definitions for APEX Navigator for Multicloud Storage

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the AwsAccountsCreateInput type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AwsAccountsCreateInput{}

// AwsAccountsCreateInput As part of setting up a role, provide the ARN of the role. This allows APEX Navigator for Multicloud Storage to make API calls and manage and deploy storage.
type AwsAccountsCreateInput struct {
	RoleArn string `json:"role_arn"`
}

type _AwsAccountsCreateInput AwsAccountsCreateInput

// NewAwsAccountsCreateInput instantiates a new AwsAccountsCreateInput object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAwsAccountsCreateInput(roleArn string) *AwsAccountsCreateInput {
	this := AwsAccountsCreateInput{}
	this.RoleArn = roleArn
	return &this
}

// NewAwsAccountsCreateInputWithDefaults instantiates a new AwsAccountsCreateInput object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAwsAccountsCreateInputWithDefaults() *AwsAccountsCreateInput {
	this := AwsAccountsCreateInput{}
	return &this
}

// GetRoleArn returns the RoleArn field value
func (o *AwsAccountsCreateInput) GetRoleArn() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.RoleArn
}

// GetRoleArnOk returns a tuple with the RoleArn field value
// and a boolean to check if the value has been set.
func (o *AwsAccountsCreateInput) GetRoleArnOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RoleArn, true
}

// SetRoleArn sets field value
func (o *AwsAccountsCreateInput) SetRoleArn(v string) {
	o.RoleArn = v
}

func (o AwsAccountsCreateInput) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AwsAccountsCreateInput) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["role_arn"] = o.RoleArn
	return toSerialize, nil
}

func (o *AwsAccountsCreateInput) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"role_arn",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAwsAccountsCreateInput := _AwsAccountsCreateInput{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varAwsAccountsCreateInput)

	if err != nil {
		return err
	}

	*o = AwsAccountsCreateInput(varAwsAccountsCreateInput)

	return err
}

type NullableAwsAccountsCreateInput struct {
	value *AwsAccountsCreateInput
	isSet bool
}

func (v NullableAwsAccountsCreateInput) Get() *AwsAccountsCreateInput {
	return v.value
}

func (v *NullableAwsAccountsCreateInput) Set(val *AwsAccountsCreateInput) {
	v.value = val
	v.isSet = true
}

func (v NullableAwsAccountsCreateInput) IsSet() bool {
	return v.isSet
}

func (v *NullableAwsAccountsCreateInput) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAwsAccountsCreateInput(val *AwsAccountsCreateInput) *NullableAwsAccountsCreateInput {
	return &NullableAwsAccountsCreateInput{value: val, isSet: true}
}

func (v NullableAwsAccountsCreateInput) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAwsAccountsCreateInput) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


